...
- [ ] **Implement Responsive Column Hiding (AC: 1, 4)**
  - [ ] Update `internal/app/table.go` to use `hide_order` from config when calculating visible columns based on `m.termWidth`.
  - [ ] Ensure the row cursor (▶) and the "primary" column (usually Name/ID) are never hidden.
  - [ ] Validate that cell truncation only happens for the flexible column, never for fixed-width columns.
- [ ] **Implement Responsive Hint Visibility (AC: 2, 3)**
  - [ ] Update `renderFooter` in `internal/app/view.go` to filter `[]Hint` based on `h.MinWidth <= m.termWidth`.
  - [ ] Implement priority-based truncation in `internal/app/hints.go` when the combined width of visible hints exceeds available space.
  - [ ] Ensure `MinWidth: 0` hints are protected from truncation.
- [ ] **Testing & Validation (AC: all)**
  - [ ] Create `internal/app/responsive_layout_test.go` to simulate various terminal widths (80, 100, 120, 150) and assert column/hint presence.
  - [ ] Verify that no layout corruption occurs at extremely narrow widths (e.g., 40 columns).
