...
func Run() {
	...
	// Always open debug/o6n.log for error logging.
	// Verbose debug output additionally requires --debug.
	os.Mkdir("debug", 0755)
	logFile, err := os.OpenFile("debug/o6n.log", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
	...
	if *debug {
		...
		log.Println("--- o6n debug session started ---")
	} else {
	...
	// Load split config files (o6n-env.yaml + o6n-cfg.yaml). No legacy fallback.
	envCfg, err := config.LoadEnvConfig("o6n-env.yaml")
	if err != nil {
		fmt.Printf("Error loading o6n-env.yaml: %v\n", err)
		fmt.Println("Please create o6n-env.yaml from the example.")
		os.Exit(1)
	}

	appCfg, err := config.LoadAppConfig("o6n-cfg.yaml")
	if err != nil {
		fmt.Printf("Error loading o6n-cfg.yaml: %v\n", err)
		os.Exit(1)
	}

	if len(envCfg.Environments) == 0 {
		log.Println("No environments configured. Please create 'o6n-env.yaml' and define at least one environment.")
		return
	}

	// Load persisted runtime state (o6n-stat.yml). Missing file is not an error.
