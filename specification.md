...
- `internal/app/` — all TUI application logic: model, update, view, nav, commands, skin, styles, table, util, edit.
- `internal/client/` — HTTP client wrapper using the generated OpenAPI client.
- `internal/config/` — Config structs and loaders for env, app and state files.
...
- `o8n-env.yaml` (runtime) — environment configuration (kept out of source control). Contains only credentials and environment definitions; no mutable runtime state.
- `o8n-cfg.yaml` — application-level configuration (table definitions, column specs, action definitions). Static; never written at runtime.
- `o8n-stat.yml` (runtime, auto-generated) — mutable runtime state: active environment, skin, latency toggle, last navigation position. Git-ignored.
...
- tests (go test unit tests) — exercises config load, API client, and TUI behaviour.
...
- **Active skin**: Stored in `o8n-stat.yml`, switchable at runtime (CLI `--skin` flag overrides)
- **Footer breadcrumb**: Environment-specific background color, up to 3 sections
- **Remote indicator**: Flash (⚡) for 0.2s on REST API calls; optional latency ms display (toggled with `L`)
...
### Default Behavior
- **Environment**: "local" (if configured) or first available; overridden by `active_env` in `o8n-stat.yml`
- **Root context**: process-definitions; restored from `navigation.root` in `o8n-stat.yml`
- **Auto-refresh**: Toggle with `r` or `<ctrl>+r` (5s interval when enabled)
- **Latency display**: Hidden by default; toggle with `L`; persisted in `o8n-stat.yml`
...
- Note: `active` and `skin` fields have been moved to `o8n-stat.yml`. Backward-compatible reads are supported but the fields are no longer written back.

### AppState (o8n-stat.yml)

Auto-generated runtime state file. Git-ignored. Updated on every navigation transition and on clean exit.

- active_env: string — currently selected environment name
- skin: string — currently selected skin name
- show_latency: bool — whether to show request latency in footer
- navigation:
  - root: string — current root context (e.g. "process-definition")
  - breadcrumb: []string — ordered list of labels (e.g. ["process-definitions", "my-process", "variables"])
  - selected_definition_key: string — key of selected process/job definition
  - selected_instance_id: string — ID of selected instance
...
## UI Rendering (main.go View)
   - Vertical layout in exact order:
     ...
     4. Footer (1 row) — three columns separated by " | ":
        ...
        - Column 3: Remote access indicator — single character; displays "⚡" for 0.2s after issuing a REST call; otherwise blank. Optionally shows request latency in ms when `show_latency` is true (toggled with `L`).
...
## Keybindings and behavior
   - `<ctrl>+c`: quit
   - `<ctrl>+e`: switch environment (cycles through configured envNames). Persists active environment to `o8n-stat.yml` (best-effort).
   - `L`: toggle latency display in footer (default: off). Persists to `o8n-stat.yml`.
